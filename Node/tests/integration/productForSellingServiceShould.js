    ...
    it('return all products with their prices', function(done){
        async.parallel([
            ...
            serverActor.productForSellingService.getAll(function(err,products){
                expect(products.some(function(x){return x._id === results[0]._id && x.price === 5.5})).to.be.true
                expect(products.some(function(x){return x._id === results[1]._id && x.price === 2})).to.be.true
                done();
            })
        });
    });
